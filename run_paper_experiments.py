...
import extra_patches  # noqa: F401 — applies monkey-patches on import
...
class LossConfig:
    ...
    @classmethod
    def from_preprocessor_config(cls, path: str,
        ...
        if overrides:
            for k, v in overrides.items():
                if hasattr(cfg, k):
                    ...
                    print(f"  ⚠ unknown LossConfig key: {k}")
        return cfg
...
# 1. Setup
# ════════════════════════════════════════════════════════════
def setup_run(cfg: ExperimentConfig):
...
def load_splits(cfg: ExperimentConfig):
    ...
    print(f"sizes: train={len(train_ds):,}  "
          f"val={len(val_ds):,}  test={len(test_ds):,}")
    print(f"Rate stats: mean={train_ds.rate_mean:.1f}, "
          f"std={train_ds.rate_std:.1f}")

    common = dict(batch_size=cfg.train.batch_size,
                  collate_fn=orig.collate_fn,
                  num_workers=cfg.run.num_workers, pin_memory=True)
...
# 2. Build model
# ════════════════════════════════════════════════════════════
def build_model(cfg: ExperimentConfig, cards, default_rate, device):
...
def build_optimizer(model, cfg: ExperimentConfig):
    ...
    static_p  = (list(model.static_head_embeddings.parameters())
    ...
    segment_p = (list(model.tariff_output_log_scale.parameters())
    ...
    head_p    = (list(model.correction_head.parameters())
    ...
    ], weight_decay=cfg.train.weight_decay, betas=(0.9, 0.98), eps=1e-8)


# ════════════════════════════════════════════════════════════
# 3. Train
# ════════════════════════════════════════════════════════════
def train(cfg: ExperimentConfig, model, train_loader, val_loader,
    ...
    early = orig.EarlyStopping(patience=cfg.train.patience, mode="min",
                               restore_best=True)
    grad_acc = orig.GradientAccumulator(1)
    ema = (orig.ExponentialMovingAverage(model, cfg.train.ema_decay)
           if cfg.train.use_ema else None)
    augmenter = (orig.TimeSeriesAugmenter(noise_std=0.05,
                                          scale_range=(0.9, 1.1),
                                          mask_prob=0.1)
                 if cfg.train.use_augmentation else None)

    logger = orig.TrainingLogger(cfg.run.checkpoint_dir, "paper_run")
    logger.log_config({
        "run":   asdict(cfg.run),
    ...
    for epoch in range(cfg.train.max_epochs):
        ...
        tr = orig.train_one_epoch(
            ...
            grad_acc, ema=ema, augmenter=augmenter,
            max_grad_norm=cfg.train.max_grad_norm,
            log_interval=cfg.run.log_interval, scaler=scaler)
        va = orig.validate(model, val_loader, loss_fn, device, epoch,
                           use_ema=cfg.train.use_ema, ema=ema)
        tr["lr"] = lr
        logger.log(tr, epoch, "train")
        logger.log(va, epoch, "val")
        logger.print_summary(epoch, tr, va, lr)
