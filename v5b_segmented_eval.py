...
def build_records(
    ...
    ----------
    npz_path : str
        Path to the input NPZ file (test or validation split).
    cards : OrderedDict
        Static feature cardinalities, used to locate the tariff column.
    tariff_idx : int
        Index of the tariff column in the static features (unused, kept
        for compatibility; column is found via cards.keys()).

    Returns
    -------
    dict of numpy arrays (columnar)
        Keys include: meter, t, tariff, dt, gap, history_len, meter_vol,
        target, pred_persist, pred_peer, rate, peer_rate.
    """
    ...
    logger.info(f"   built {len(out['target']):,} reading-records from {N:,} meters")
    return out
...
# ════════════════════════════════════════════════════════════
def compute_tariff_mean_consumption(
    train_npz: str,
    cards: OrderedDict,
) -> Tuple[Dict[int, float], float]:
    """
    Mean consumption (rate*dt) per tariff, computed from training readings.
    ...
    means : dict[int, float]
        Mapping from tariff code to mean consumption (kWh) for that tariff.
    global_mean : float
        Overall mean consumption across all tariffs.
    """
...
def compute_model_predictions(
    ...
    same records as build_records.

    Parameters
    ----------
    checkpoint : str
        Path to the model checkpoint (e.g., best_ema.pt).
    npz_path : str
        Path to the input NPZ file (test or validation split).
    cards_path : str
        Path to the JSON file with static cardinalities.
    agg_path : str
        Path to the pickle file with peer_avg aggregates.

    Returns
    -------
    dict[(meter_index, time_index) -> float]
        Predicted consumption (kWh) for each valid reading.
    """
    import torch
    ...
    model = p.ImprovedZINBElectricityMeterEncoder(
        ...
        use_time_aware_attention=False, default_rate=4.6,
        aggregates_pkl=agg_path,
    ).to(device)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            meter_offset += bsz
    logger.info(f"   model produced {len(preds_by_key):,} predictions")
    return preds_by_key
...
# Reporting helpers
# ════════════════════════════════════════════════════════════
def mae(pred: npt.NDArray[np.float64], target: npt.NDArray[np.float64]) -> float:
    """
...
def report_partition(
    rec: Dict[str, npt.NDArray[np.float64]],
    model_pred: Optional[npt.NDArray[np.float64]] = None,
    gap_easy: float = 70,
    vol_easy: float = 3.0,
    hist_easy: int = 5,
) -> None:
    """
    Print MAE by different partitions (gap, volatility, history length)
    and for the well‑conditioned core.

    Parameters
    ----------
    rec : dict
        Dictionary of records as returned by build_records.
    model_pred : ndarray or None
        Array of model predictions (aligned to rec). If None, use persistence.
    gap_easy : float, default=70
        Maximum gap (days) to be considered "easy".
    vol_easy : float, default=3.0
        Maximum rate volatility (kWh/30min) to be considered "easy".
    hist_easy : int, default=5
        Minimum history length (number of prior valid readings) to be considered "easy".
    """
    target = rec["target"]
    gap = rec["gap"]
    vol = rec["meter_vol"]
    ...
    logger.info("\n" + "=" * 64)
    logger.info("(A) WELL-CONDITIONED SUBSET REFRAMING")
    logger.info("=" * 64)
    logger.info(f"Easy thresholds: gap<{gap_easy}d, meter_vol<{vol_easy}, "
                f"history>={hist_easy}")

    def show(name: str, sel: npt.NDArray[np.bool_], pred: npt.NDArray[np.float64]) -> None:
        n = int(sel.sum())
        if n == 0:
            logger.info(f"  {name:<34} n=0")
            return
        m = mae(pred[sel], target[sel])
        frac = 100 * n / len(target)
        mean_t = float(target[sel].mean())
        logger.info(f"  {name:<34} n={n:>8,} ({frac:4.1f}%)  "
    ...
    logger.info(f"  Hard contributes {100*err[~core].sum()/total:4.1f}% of total "
                f"abs error from {100*(~core).mean():4.1f}% of readings")


def report_baselines(
    rec: Dict[str, npt.NDArray[np.float64]],
    tariff_means: Dict[int, float],
    global_mean: float,
    model_pred: Optional[npt.NDArray[np.float64]] = None,
) -> None:
    """
    Print MAE and RMSE for baseline predictors and optionally the transformer.
    ...
    rec : dict
        Dictionary of records as returned by build_records.
    tariff_means : dict[int, float]
        Per-tariff mean consumption from training.
    global_mean : float
        Global mean consumption from training.
    model_pred : ndarray or None
        Array of model predictions (aligned to rec). If None, skip transformer row.
    """
    target = rec["target"]
    tariff = rec["tariff"]

    logger.info("\n" + "=" * 64)
    logger.info("(B) BASELINE COMPARISON (same readings)")
    logger.info("=" * 64)
    ...
    logger.info(f"  {'predictor':<34} {'MAE':>9} {'RMSE':>9}")
    logger.info("  " + "-" * 54)
    for name, pred in rows:
        m = mae(pred, target)
        r = float(np.sqrt(np.mean((pred - target) ** 2)))
        logger.info(f"  {name:<34} {m:>9.1f} {r:>9.1f}")
...
# ════════════════════════════════════════════════════════════
def main() -> None:
    """Parse arguments, build records, compute baselines, and optionally load model."""
    ...
    ap.add_argument("--save_pred", default=None,
                    help="path to write an .npz of transformer predictions "
                         "aligned to deployment_reframing's reading order, "
                         "so it can be fed in via --pred_npz")
    args = ap.parse_args()
    ...
    logger.info("Computing tariff-mean baseline from train ...")
    ...
    if args.checkpoint:
        ...
        if not ok.all():
            logger.warning(f"   WARNING: {int((~ok).sum())} records lack model pred; "
                           f"dropping")
            for k in rec:
                rec[k] = rec[k][ok]
    ...
    # Optionally save transformer predictions for the deployment figure.
    # We save BOTH a flat pred array (aligned to this script's records)
    # AND the (meter, t) keys, so deployment_reframing.py can align by key
    # regardless of small differences in reading-enumeration edge cases.
    if args.save_pred and model_pred is not None:
        np.savez_compressed(
            args.save_pred,
            pred=model_pred.astype(np.float64),
            meter=rec["meter"].astype(np.int64),
            t=rec["t"].astype(np.int64),
            target=rec["target"].astype(np.float64),
            gap=rec["gap"].astype(np.float64),
            vol=rec["meter_vol"].astype(np.float64),
            hist=rec["history_len"].astype(np.int64),
            tariff=rec["tariff"].astype(np.int64),
        )
        logger.info(f"\n  Saved {len(model_pred):,} transformer predictions "
                    f"(with keys + meta) -> {args.save_pred}")
        logger.info(f"  Feed into the deployment figure with:")
        logger.info(f"    python deployment_reframing.py "
                    f"--pred_npz {args.save_pred} --eval {args.npz}")
